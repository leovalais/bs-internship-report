...
\titleformat{\section}{\Large\bf\clearpage}{\thesection}{1em}{}[\color{gray}\titlerule]
...
\renewenvironment{quote}{%
  \begin{center}
    \bigquotemark{\og}
    \begin{minipage}{.75\textwidth}\small\it\color{gray!50!dark}
    }{%
    \end{minipage}
    \bigquotemark{\fg}
  \end{center}}
...
**** TODO L'extension {{{wikiext(client-server-switch)}}}
...
- le menu latéral rétractable (extension {{{wikiext(drawer)}}}), aussi identique
  pour toutes les pages du site[fn:16], et enfin,
...
#+begin_ex
Dans le fichier ~template.wiki~ du projet Eliom, l'on a[fn:17] :
#+BEGIN_SRC text
...
est donc /wiki_in_template/[fn:18], abbrégé =wit=.
...
listing [[lst:c1-c2-template]][fn:19].
...
*** Générer une version d'un projet : doc_of_project<<sec:cdc-dop>>
**** Objectifs
**** Interface
*** Gestion de la documentation du project Ocsigen
*** Intégration continue : Travis {{{acr(Ci)}}}
* Annexes
...
- Ben Kenobi :: Jedi
- Général Grievous :: Sith
- Anakin Skywalker/Dark Vador :: C'est compliqué...
...
| ~__garageline__~         | Soulignement         | _garageline_       |
| ~/-text-/~               | Barrage              | +text+             |
|--------------------------+----------------------+--------------------|
#+end_table
...
[fn:4] Cela est dû au fonctionnement de GitHub Pages. Pour plus d'information consulter l'annexe TODO sur GitHub Pages.

[fn:5] GitHub Pages permet de définir des redirections sous forme de liens symboliques. Consulter l'annexe TODO pour plus d'informations.
...
TODO sur GitHub Pages pour plus de détail.
...
[fn:16] Le site web est dit /responsive/, c'est-à-dire qu'il s'adapte aux différentes résolutions d'appareils (laptop,
...
[fn:17] La syntaxe ~@@class="cls"@@~ permet d'ajouter une classe {{{html}}} à l'élément suivant. Voir l'annexe [[sec:anx-wikicreole]]
sur le wikicréole pour plus d'informations.

[fn:18] Une légère adaptation de la convention de nommage ~A_of_B~ d'{{{ocaml}}} ("wiki_of_template" n'aurait pas eu beaucoup
de sens puisque les deux sont paramètres de l'outil).

[fn:19] Afin de simplifier l'écriture de celle-ci la commande du listing [[lst:c1-c2-template]] utilise une fonctionnalité
