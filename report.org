...
#+LATEX_HEADER: \usepackage{codebox/codebox}
  ...
  CONTENTS is nil.  INFO is a plist holding contextual information."
    (let ((attr (org-export-read-attribute :attr_latex horizontal-rule))
      ...
      (concat
       ...
       "\\bigskip\\noindent\\textcolor{gray}{\\small\\centerline{*\\,*\\,*}}\\medskip")))
#+END_SRC

#+NAME: setup-latex-env
#+BEGIN_EXPORT latex
...
- supprimer les cadres autour des liens
#+END_COMMENT
...
Une fois cela terminé, mon travail se portera davantage vers la platefome BeSport,
nottament en ce qui concerne /l'importation de données/.
...
*** html_of_wiki
C'est dans cette optique qu'est né le projet html_of_wiki[fn:3] (abbrégé =how=).
...
- éventuellement rendre =ohow= suffisament indépendant de l'infrastructure
...
Le tableau [[tab:ext-to-support]] liste et décrit les extensions (et leur attributs[fn:6])
définies pour le projet Ocsigen que =ohow= doit pourvoir supporter[fn:7].
...
La fonction de one_html_of_wiki est /simple/ et /unique/[fn:8]:
#+BEGIN_QUOTE
...
- Les liens générés doivent être _tous_[fn:9] relatifs.
...
- Il doit y avoir une option ~--print~ pour afficher le code {{{html}}} résultant
  dans la console (sortie standard) afin de pouvoir être composé[fn:10].
...
sont les suivantes :
- ~a_manual~ qui décrit un lien vers un chapitre[fn:12] du manuel,
- ~a_api~, ~a_api_type~, ~a_api_code~ qui décrivent un lien vers la page de
  documentation d'un symbole d'une {{{api}}}\(\)[fn:13],
...
- À la racine de chaque version doit se trouver un lien symbolique nommé
  ~index.html~[fn:14] qui pointe vers la page d'introduction ou de présentation
...
- Dans l'idée de rester le plus générique possible, =ohow= accepte les options
  ...
  - ~--api~ désigne le dossier contenant les wikis de l'{{{api}}} (~a_api*~[fn:15]),
...
#+begin_ex
L'invocation de =ohow= suivante décrit l'architecture illustrée par la figure
[[fig:ex-arch-ext]].
#+BEGIN_SRC shell
...
#+NAME: fig:ex-arch-ext
#+CAPTION: Exemple d'architecture compatible avec l'utilisation des extensions ~a_*~.
#+begin_figure
TODO
#+end_figure
...
#+INCLUDE: "eliom-menu.wiki" src text
...
en deux versions : une version /client/ et une version /serveur/[fn:16]. C'est par exemple
...
contenu de chaque wiki, nottament pour des questions de mise en page.
...
- le menu latéral rétractable (extension {{{wikiext(drawer)}}}), aussi identique
  pour toutes les pages du site[fn:17], et enfin,
...
#+begin_ex
Dans le fichier ~template.wiki~ du projet Eliom, l'on a[fn:18] :
#+BEGIN_SRC text
...
qu'une seule chose[fn:8], il a été décidé que la fonction d'insetion de /contenu/
...
est donc /wiki_in_template/[fn:19], abbrégé =wit=.
...
listing [[lst:c1-c2-template]][fn:20].
...
*** TODO Générer une version d'un projet : doc_of_project<<sec:cdc-dop>>
**** Objectifs
**** Interface
...
- gestion des liens symboliques faisant office de redirections[fn:21] automatique.
...
[[tab:abbr]] les abbréviations disponibles.
...
#+NAME: tab:abbr
#+CAPTION: Syntaxes d'abbréviations de liens ajoutées par le projet Ocsigen.
#+begin_table
|---------------------------+------------------------------+-----------------------------------------------|
| Abbréviation              | Équivalence                  | Description                                   |
|---------------------------+------------------------------+-----------------------------------------------|
...
[fn:3] Il existe en {{{ocaml}}} une convention de nommage pour les fonctions de conversion. Par exemple, la fonction standard
...
[fn:4] Cela est dû au fonctionnement de GitHub Pages. Pour plus d'information consulter l'annexe [[sec:anx-ghpages]] sur GitHub Pages.

[fn:5] GitHub Pages permet de définir des redirections sous forme de liens symboliques. Consulter l'annexe [[sec:anx-ghpages]]
pour plus d'informations.

[fn:6] Toutes les extensions acceptent les attributs ~class~ et ~id~ correspondant aux attributs {{{html}}} de même nom.

[fn:7] Les descriptions données sont simplement indicatives. Les extensions complexes seront décrites avec plus de précision
lors de l'explication des problèmes techniques qu'elles causent.

[fn:8] Cela permet de rendre le programme plus simple à maintenir mais aussi plus en accord avec la philosophie
Unix. "Make each program do one thing well."

[fn:9] Ceci ne concerne que les liens entre les pages du projet. Les liens vers d'autres sites web sont nécéssairement absolus.

[fn:10] Une utilisation idomatique de la ligne de commande des systèmes Unix est de composer différents programmes
...
[fn:12] L'on parle de "chapitre" pour désigner les pages web du manuel et de "page" pour désigner les pages web de(s) {{{api}}}(s),
mais les deux termes désignent bien tous deux des pages {{{html}}} classique.

[fn:13] /Application Programming Interface/ ou "interface de programmation applicative", désigne l'ensemble
...
[fn:14] Il y a également un mécanisme (désirable) intégré à GitHub Pages concernant ~index.html~. Consulter l'annexe
[[sec:anx-ghpages]] sur GitHub Pages pour plus de détail.

[fn:15] La notation ~foo*~ est tirée de la syntaxe shell (plus spécifiquement du /globbing/) afin de désigner tous les fichiers
...
[fn:16] C'est une conséquence de la /programmation multi-tiers/. Voir l'annexe [[sec:anx-pmt]] sur ce style de programmtion pour
plus d'informations.
